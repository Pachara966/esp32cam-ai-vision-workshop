#!/usr/bin/env python3
"""
Generate Workshop_v2.pptx — ESP32-CAM AI Vision Workshop (RMUTT)
Usage : python generate_pptx_v2.py   (run from inside slides/ directory)
Output: Workshop_v2.pptx  (~60 slides, clean-white light theme, bilingual TH/EN)
"""
...
# ── Colour palette (Clean White light theme) ──────────────────────────────────
PAGE      = RGBColor(0xFF, 0xFF, 0xFF)   # slide background — white
PANEL     = RGBColor(0xEE, 0xF3, 0xF8)   # cards / code panels — light gray
GHOST     = RGBColor(0xE3, 0xEC, 0xF4)   # section-number watermark (faint)
CYAN      = RGBColor(0x00, 0x95, 0xC8)   # primary accent (darkened for white bg)
ORANGE    = RGBColor(0xE8, 0x5A, 0x2A)   # lab / callout accent (darkened)
INK       = RGBColor(0x14, 0x26, 0x3A)   # primary text / titles on background
BODY      = RGBColor(0x2B, 0x3B, 0x4D)   # body text
MUTED     = RGBColor(0x5E, 0x73, 0x87)   # secondary / italic text
GREEN     = RGBColor(0x1F, 0xA5, 0x4A)   # success / checkmarks (darkened)
ON_ACCENT = RGBColor(0xFF, 0xFF, 0xFF)   # text on coloured fills (stays white)
CODE_TXT  = RGBColor(0x0B, 0x6B, 0x3A)  # monospace code text — dark green

# Aliases so the bulk of the slide functions below need no edits
BG    = PAGE
BG2   = PANEL
LIGHT = BODY
GRAY  = MUTED
...
def _lab_badge(slide, num, color=ORANGE):
    ...
    _tb(slide, f"LAB  {num:02d}", 11.0, 0.18, 2.1, 0.62,
        size=18, color=ON_ACCENT, bold=True, align=PP_ALIGN.CENTER)
...
def _code_box(slide, code, left, top, width, height):
    """Light monospace code panel."""
    _box(slide, left, top, width, height, PANEL, border=True)
    tb = slide.shapes.add_textbox(
...
def _step_badge(slide, step_num, label="", color=ORANGE):
    ...
    _tb(slide, str(step_num), 0.5, 1.6, 0.7, 0.7,
        size=22, color=ON_ACCENT, bold=True, align=PP_ALIGN.CENTER)
    if label:
...
def _checklist(slide, items, left=0.8, top=2.0, size=20):
    ...
    lines = []
    for item in items:
        lines.append((f"✅  {item}", INK, size, False))
...
def s00_cover(prs):
    ...
    _box(slide, 3.5, 2.95, 6.33, 0.05, MUTED)   # divider
...
def s00_overview(prs):
    ...
    _multiline(slide, [
        ...
        ("  วิเคราะห์ภาพ  •  นับวัตถุ  •  จำแนกประเภท  •  ตอบคำถาม", BODY, 18, False),
    ], 0.5, 1.85, 12.5, 0.9, gap=4)

    # Flow diagram
    _box(slide, 0.5, 2.95, 12.3, 3.8, PANEL, border=True)
    flow = [
        ("📷  ESP32-CAM",        CYAN,   20, True),
        ("        │",            MUTED,  16, False),
        ("        ▼",            MUTED,  16, False),
        ("📶  Wi-Fi  →  /capture endpoint",  BODY, 18, False),
        ("        │",            MUTED,  16, False),
        ("        ▼",            MUTED,  16, False),
        ("⚙️   FastAPI Backend  (Python)",    BODY, 18, False),
        ("        │",            MUTED,  16, False),
        ("        ▼",            MUTED,  16, False),
...
def s00_agenda(prs):
    ...
    for time, activity, icon in rows:
        col = CYAN if "Lab" in activity or "Lecture" in activity else BODY
        bd  = "Lab" in activity or "Lecture" in activity
        _tb(slide, time,     0.4, top, 2.3, 0.38, size=14, color=MUTED)
...
def s01_iot(prs):
    ...
    for icon_title, desc in examples:
        _box(slide, 0.5, top, 12.3, 1.1, PANEL, border=True)
...
def s01_esp32cam(prs):
    ...
    _box(slide, 0.5, 1.7, 7.5, 5.3, PANEL, border=True)
    top = 1.9
...
def s01_wifi(prs):
    ...
    for label, desc in points:
        _box(slide, 0.5, top, 12.3, 0.82, PANEL)
...
def s02_http(prs):
    ...
    for i, (method, color, use, example) in enumerate([
        ...
        left = 0.5 + i * 6.6
        _box(slide, left, 1.75, 6.3, 5.4, PANEL, border=True)
        _box(slide, left, 1.75, 6.3, 0.62, color)
        _tb(slide, method, left + 0.2, 1.78, 5.9, 0.55,
            size=28, color=ON_ACCENT, bold=True)
        ...
        _tb(slide, "ไม่มี body / URL parameters",
            ...
            size=13, color=MUTED, italic=(i == 0))
    ...
    sample = """\
{
  ...
  "result": "ภาพแสดงโต๊ะทำงานที่มีโน้ตบุ๊ค 1 เครื่อง
    ...
    _multiline(slide, [
        ...
        ("• ใช้  { }  สำหรับ object", BODY, 16, False),
        ("• ใช้  [ ]  สำหรับ list/array", BODY, 16, False),
        ("• Key ใส่ใน  \" \"  เสมอ", BODY, 16, False),
        ("• Value อาจเป็น string, number,", BODY, 16, False),
        ("  boolean, list หรือ object", BODY, 16, False),
        ("• ทุก API ใน Workshop นี้", BODY, 16, False),
        ("  ตอบกลับเป็น JSON", BODY, 16, False),
...
def s03_ai(prs):
    ...
    stages = [
        ("Rule-Based",   "กฎตายตัว\nIF วัตถุกลม AND สีแดง → THEN แอปเปิ้ล", MUTED),
...
def s03_genai(prs):
    ...
    for icon_title, desc in items:
        ...
        _box(slide, 0.5, top, 12.3, 1.05,
             RGBColor(0xE4, 0xF6, 0xE9) if highlight else PANEL,
             border=highlight)
...
def s03_vision(prs):
    ...
    for i, (cap, example) in enumerate(capabilities):
        ...
        row = top + (i // 2) * 1.38
        _box(slide, 0.5 + col, row, 6.3, 1.15, PANEL, border=True)
...
def s03_gemini(prs):
    ...
    _box(slide, 0.5, 1.75, 12.3, 5.4, PANEL, border=True)
    info = [
...
def s03_prompt(prs):
    ...
    # Formula box — light tint
    _box(slide, 0.5, 1.72, 12.3, 0.72, RGBColor(0xE2, 0xF1, 0xF8), border=True)
    ...
    for label, example, desc in parts:
        _box(slide, 0.5, top, 12.3, 1.02, PANEL, border=True)
...
def s04_header(prs):
    ...
    _tb(slide, "ติดตั้งกล้อง • Flash Firmware • ดู Live Stream", 4.5, 3.55, 8.5, 0.6,
        size=18, color=BODY)
    _tb(slide, "เวลา: ~60 นาที", 4.5, 4.3, 4.0, 0.4, size=15, color=MUTED)
...
def s04_step1(prs):
  ...
  ต่อกับ Notebook / PC"""
    ...
    _multiline(slide, [
        ...
        ("ต้องทำก่อน Upload ทุกครั้ง", BODY, 16, False),
        ("หลัง Upload เสร็จ:", BODY, 16, False),
...
def s04_step3(prs):
    ...
    _tb(slide, "คัดลอก  secrets.example.h  →  secrets.h  แล้วแก้ไข:",
        0.5, 2.42, 12.5, 0.45, size=16, color=BODY)
...
7. กด RST อีกครั้ง → ESP32-CAM บูตปกติ"""
    ...
    _multiline(slide, [
        ...
        ("❌ ค้างที่ Connecting", BODY, 15, False),
        ("→ GPIO0 ไม่ได้ต่อ GND", MUTED, 14, False),
        ("❌ Upload failed", BODY, 15, False),
        ("→ เลือก Port ผิด", MUTED, 14, False),
        ("❌ ไม่เห็น Port", BODY, 15, False),
        ("→ ไม่มีไดรเวอร์ CH340", MUTED, 14, False),
...
Camera Ready! Stream: http://192.168.x.x:81/stream"""
    ...
    _multiline(slide, [
        ...
        ("http://<IP>", BODY, 15, False),
        ("→ หน้า Web UI", MUTED, 14, False),
        ("http://<IP>/capture", BODY, 15, False),
        ("→ ดาวน์โหลด JPEG", MUTED, 14, False),
        ("http://<IP>:81/stream", BODY, 15, False),
        ("→ Live video stream", MUTED, 14, False),
...
# เห็น (.venv) หน้า prompt = สำเร็จ ✅"""
    ...
    _tb(slide, "💡  Virtual Environment แยก Python packages ของ project ออกจากระบบ",
        0.5, 5.75, 12.5, 0.45, size=14, color=MUTED, italic=True)
...
def s05_step3(prs):
    ...
    _tb(slide, "คัดลอก  .env.example  →  .env  แล้วแก้ไข:",
        0.5, 2.42, 12.5, 0.4, size=16, color=BODY)
...
def s06_header(prs):
    ...
    _tb(slide, "API Key จริง • ภาพจากกล้อง • Prompt Engineering", 4.5, 3.55, 8.5, 0.6,
        size=18, color=BODY)
    _tb(slide, "เวลา: ~75 นาที", 4.5, 4.3, 4.0, 0.4, size=15, color=MUTED)
...
def s06_step4(prs):
    ...
    for label, prompt in prompts:
        _box(slide, 0.5, top, 12.3, 1.3, PANEL, border=True)
...
def s07_overview(prs):
    ...
    for step, desc in flow:
        _box(slide, 0.5, top, 12.3, 0.95, PANEL, border=True)
...
def s07_step1(prs):
    ...
    for icon_name, desc in projects:
        ...
        _tb(slide, desc, 5.4, top, 7.7, 0.42, size=15, color=BODY)
        top += 0.75
...
ตอบเป็นภาษาไทย"""
    ...
    _multiline(slide, [
        ("Tips:", CYAN, 17, True),
        ("• ทดสอบอย่างน้อย 3 ครั้ง", BODY, 15, False),
        ("• ถ้า AI ตอบกว้างเกิน", BODY, 15, False),
        ("  → เพิ่มรายละเอียดใน Format", MUTED, 14, False),
        ("• ถ้า AI ตอบไม่ตรง", BODY, 15, False),
        ("  → ปรับ Role ให้ชัดขึ้น", MUTED, 14, False),
        ("• ดูตัวอย่างใน", BODY, 15, False),
...
def s07_tips(prs):
    ...
    top = 1.9
    for icon_title, desc in items:
        _box(slide, 0.5, top, 12.3, 1.15, PANEL, border=True)
        _tb(slide, icon_title, 0.75, top + 0.1, 3.0, 0.5,
            size=19, color=CYAN, bold=True)
        _tb(slide, desc, 3.9, top + 0.1, 9.0, 0.7, size=16, color=BODY)
        top += 1.32
...
def s09_careers(prs):
    ...
    for i, (title, desc) in enumerate(careers):
        col = 0 if i % 2 == 0 else 6.6
        row = top + (i // 2) * 1.6
        _box(slide, 0.5 + col, row, 6.3, 1.38, PANEL, border=True)
...
def s09_resources(prs):
    ...
    for icon_title, url in resources:
        _box(slide, 0.5, top, 12.3, 0.88, PANEL, border=True)
...
def s09_summary(prs):
    ...
    for lab, desc, color in summary:
        _box(slide, 0.5, top, 12.3, 1.05, PANEL, border=True)
...
def s09_qa(prs):
    ...
    _tb(slide, "Q&A", 0.5, 2.0, 12.33, 2.5,
        size=100, color=INK, bold=True, align=PP_ALIGN.CENTER)
...
def s09_thankyou(prs):
    ...
    _tb(slide, "ขอบคุณทุกคน!", 0.5, 0.9, 12.33, 1.5,
        size=60, color=INK, bold=True, align=PP_ALIGN.CENTER)

    _tb(slide, "Thank You!", 0.5, 2.3, 12.33, 1.0,
        size=40, color=CYAN, bold=True, align=PP_ALIGN.CENTER)

    _box(slide, 3.5, 3.45, 6.33, 0.06, MUTED)

    _tb(slide, "มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรี (RMUTT)",
        0.5, 3.65, 12.33, 0.55,
        size=19, color=BODY, align=PP_ALIGN.CENTER)
...
def main():
    ...
    print("Building slides (light theme v2)...")
